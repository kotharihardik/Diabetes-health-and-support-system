...
const Suggestions2 = () => {
  ...
  const generateSuggestions = (formData) => {
    ...
    if (formData.Pregnancies > 5) {
        suggestions.push("Multiple pregnancies increase the risk of gestational diabetes. Regular screening is recommended.");
    }

    // Glucose Levels
    if (formData.Glucose < 70) {
        suggestions.push("Your glucose levels are low, which may lead to hypoglycemia. Consider consulting a healthcare professional.");
    } else if (formData.Glucose >= 70 && formData.Glucose < 100) {
        suggestions.push("Your glucose levels are in the normal range. Maintain a balanced diet to support healthy glucose levels.");
    } else if (formData.Glucose >= 100 && formData.Glucose < 126) {
        suggestions.push("You have borderline high glucose levels. Regular physical activity and a low-sugar diet may help.");
    } else if (formData.Glucose >= 126) {
        suggestions.push("High glucose levels could indicate diabetes. Consult with a healthcare provider for further evaluation.");
    }
    ...
    if (formData.BloodPressure < 90) {
        ...
        suggestions.push("Your blood pressure is in the healthy range. Keep up the good work with regular physical activity.");
    } else if (formData.BloodPressure > 120 && formData.BloodPressure <= 140) {
        suggestions.push("Your blood pressure is slightly elevated. Reducing sodium intake and managing stress may help.");
    } else if (formData.BloodPressure > 140) {
        suggestions.push("Your blood pressure is high. It's advisable to consult a doctor and consider lifestyle changes.");
    }

    // Skin Thickness
    if (formData.SkinThickness < 10) {
        suggestions.push("Low skin thickness measurement could indicate a lack of subcutaneous fat. Consider improving your diet.");
        ...
        suggestions.push("High skin thickness may suggest increased body fat. Monitoring your diet and exercise could help.");
    }

    // Insulin Levels
    if (formData.Insulin < 16) {
        suggestions.push("Low insulin levels could suggest an issue with insulin production. Consult with a healthcare provider.");
    } else if (formData.Insulin >= 16 && formData.Insulin < 166) {
        suggestions.push("Insulin levels are within a healthy range. Maintain a balanced diet to support healthy insulin function.");
    } else if (formData.Insulin >= 166) {
        suggestions.push("High insulin levels could indicate insulin resistance or diabetes. Consider further medical evaluation.");
    }

    // BMI (Body Mass Index)
    if (formData.BMI < 18.5) {
        suggestions.push("Your BMI indicates you are underweight. A balanced diet rich in nutrients is recommended.");
    } else if (formData.BMI >= 18.5 && formData.BMI <= 24.9) {
        suggestions.push("Your BMI is within the normal range. Keep up with regular physical activity and a balanced diet.");
    } else if (formData.BMI >= 25 && formData.BMI <= 29.9) {
        suggestions.push("You are in the overweight range. Consider increasing physical activity and watching your caloric intake.");
    } else if (formData.BMI >= 30) {
        suggestions.push("Your BMI is in the obese range. A healthcare provider can help create a weight management plan.");
    }

    // Diabetes Pedigree Function
    if (formData.DiabetesPedigreeFunction < 0.5) {
        suggestions.push("Your family history indicates a lower risk of diabetes. Continue maintaining a healthy lifestyle.");
    } else if (formData.DiabetesPedigreeFunction >= 0.5) {
        suggestions.push("Your family history suggests a higher risk of diabetes. Regular monitoring and a healthy lifestyle are essential.");
    }

    // Age
    if (formData.Age < 35) {
        suggestions.push("At your age, focusing on preventive measures like regular exercise and a balanced diet can significantly reduce health risks.");
    } else if (formData.Age >= 35 && formData.Age <= 50) {
        suggestions.push("This is a critical time to monitor health markers. Regular checkups and a healthy lifestyle can help prevent long-term health issues.");
    } else if (formData.Age > 50) {
        suggestions.push("As you age, regular health screenings, a balanced diet, and maintaining an active lifestyle are crucial.");
    }
