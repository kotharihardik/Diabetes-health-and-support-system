...
const ordinal_mappings = {
  ...
  physical_intensity_work: { 'Not physically intensive': 0, 'Lightly physically intensive': 1, 'Moderately physically intensive': 2, 'Very physically intensive': 3 },
...
const encodeUserData = (userData) => {
  ...
  const encodingArray = [
    ...
    ...['Gym', 'None', 'Running', 'Sports', 'Walking'].map(activity =>
      userData.physicalActivity.includes(activity) ? 1 : 0
    ),
...
const Model1 = () => {
    ...
    if (["physicalActivity", "sleepIssues", "symptoms", "otherMedicalConditions"].includes(name)) {
      ...
      if (value === "None") {
        ...
        updatedValues = checked
          ? formData[name].filter((item) => item !== "None").concat(value)
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.post('http://localhost:5000/api/predict/model1', { ...encodingArray, email });
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div>
            ...
            <div className="mb-6">
              ...
              <div className="grid grid-cols-2 gap-4 ml-6 "> {/* grid with 2 columns and some gap between items */}
                {[
          ...
          <div className=" mb-6">
            <label className="block font-bold">
              Family History of Diabetes (How many family members have or had diabetes?) *
            </label>
            <input
              type="number"
              name="familyHistory"
...
//       <p>This is the Model1 page of the MERN app.</p>
//     </div>
//   );
// };
